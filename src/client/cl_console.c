...
void Con_CheckResize (void)
{
	...
	if (cls.glconfig.vidWidth) {
		...
		g_console_field_width = g_consoleField.widthInChars =
                    width - Q_PrintStrlen(cl_consolePrompt->string) - (com_timestamps->integer ? 15 : 5);
    } else {
		width = 0;
	}
...
void Con_Init (void) {
	...
	scr_conColorRed = Cvar_Get ("scr_conColorRed", "1", CVAR_ARCHIVE);
	scr_conColorBlue = Cvar_Get ("scr_conColorBlue", "0", CVAR_ARCHIVE);
	scr_conColorGreen = Cvar_Get ("scr_conColorGreen", ".4235", CVAR_ARCHIVE);

	scr_conHeight = Cvar_Get ("scr_conHeight", "50", CVAR_ARCHIVE);
	
	scr_conBarSize = Cvar_Get ("scr_conBarSize", "2", CVAR_ARCHIVE);
	
	scr_conBarColorAlpha = Cvar_Get ("scr_conBarColorAlpha", ".75", CVAR_ARCHIVE);
	scr_conBarColorRed = Cvar_Get ("scr_conBarColorRed", "0", CVAR_ARCHIVE);
...
void Con_DrawInput (void) {
  ...
  {
    Com_sprintf( prompt,  sizeof( prompt ), "^0[^3%02d^0:^3%02d^0:^3%02d^0]^7  %s", realtime.tm_hour,
                        realtime.tm_min, realtime.tm_sec, cl_consolePrompt->string );
  }
  else
    Com_sprintf( prompt,  sizeof( prompt ), "^7%s", cl_consolePrompt->string );
...
//	qhandle_t		conShader;
    ...
    color[1] = scr_conColorGreen->value;
    color[2] = scr_conColorBlue->value;
    color[3] = 2 * frac * scr_conColorAlpha->value;
		SCR_FillRect(10, 10, 620, 460 * scr_conHeight->integer * 0.01, color);

    color[0] = scr_conBarColorRed->value;
    color[1] = scr_conBarColorGreen->value;
    color[2] = scr_conBarColorBlue->value;
    color[3] = 2 * frac * scr_conBarColorAlpha->value;
