		...
		if( FS_Initialized() && LogFileOpened == qtrue ) {
			if( cl.serverTime > 0 ) {
				...
				if( cl_logs->integer == 1 ) {
					...
					//just game time
					//do timestamp prep
					sprintf( Timestamp, "[%d:%02d:%02d]", cl.serverTime / 60000, ( cl.serverTime / 1000 ) % 60, ((cl.serverTime % 1000 ) * 60) / 1000 ); //server Time
					...
					qtime_t now;
					Com_RealTime( &now );
					sprintf( Timestamp, "[%d:%02d:%02d]", now.tm_hour, now.tm_min, now.tm_sec ); //client Time
					...
					qtime_t now;
					Com_RealTime( &now );
					sprintf( Timestamp, "[%d:%02d:%02d][%d:%02d:%02d]", now.tm_hour, now.tm_min, now.tm_sec, cl.serverTime / 60000, ( cl.serverTime / 1000 ) % 60, ((cl.serverTime % 1000 ) * 60) / 1000 ); //client Time, server Time
				}
				//decolor the string
				Q_strncpyz( NoColorMsg, text, sizeof(NoColorMsg) );
				Q_CleanStr( NoColorMsg );
