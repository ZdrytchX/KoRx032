...
vmCvar_t  g_noguidnobuild;

vmCvar_t  g_adminstring;

vmCvar_t  g_randommappercent;
...
static cvarTable_t   gameCvarTable[ ] =
{
  ...
  { &g_buildLogMaxLength, "g_buildLogMaxLength", "100", CVAR_ARCHIVE, 0, qfalse  },
  ...
  { &g_websiteSpamMessage1, "g_websiteSpamMessage1", "^2K^7nights^2o^7f^2R^7eason.net", CVAR_ARCHIVE, 0, qfalse},
  { &g_websiteSpamMessage2, "g_websiteSpamMessage2", "^7For Updates, Forums and More", CVAR_ARCHIVE, 0, qfalse},
  
  { &g_noguidnobuild, "g_noguidnobuild", "1", CVAR_ARCHIVE, 0, qfalse},
  
  { &g_adminstring, "g_adminstring", "^1~", CVAR_ARCHIVE, 0, qfalse},

  { &g_randommappercent, "g_randommappercent", "65", CVAR_ARCHIVE, 0, qfalse}
};
...
void G_CalculateBuildPoints( void )
{
 ...
 if( g_extendvote.integer )
 {
      ...
      && level.extremeSuddenDeath )
  {
              ...
              && !level.vesd )
    {
      ...
      {        
        ...
        if( !ent->client || ent->client->sess.spectatorState != SPECTATOR_NOT )
          continue;
        ...
        if( ent->client->ps.stats[ STAT_TEAM ] != TEAM_NONE )
        {
          ent->nextRegenTime = level.time
            ...
            / ( (float) ent->client->ps.stats[ STAT_MAX_HEALTH ]) ) );
          // set it to current time a little more
          // to allow vesd to start at the same time for everyone
        }
        if( ( ent->client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS )
            && BG_InventoryContainsUpgrade( UP_MEDKIT, ent->client->ps.stats ) )
        {
  ...
  if( g_extremeSuddenDeath.integer && !level.extremeSuddenDeath )
  {
    ...
    if( !g_friendlyBuildableFire.integer && g_tkmap.integer )
    {
      level.resetfbf = qtrue;
      trap_Cvar_Set( "g_friendlyBuildableFire", "1" );
    }
    ...
    for( i = 0; i < MAX_CLIENTS; i++ )
    {
      if( level.clients[ i ].ps.stats[ STAT_TEAM ] == TEAM_HUMANS )
        ...
        level.clients[ i ].pers.Credit = ALIEN_CREDITS_PER_FRAG*10; //Frags are 400 each, 10*400 = 4000, extra for adv. rant
    }
  ...
  {
    int timeleft;
    timeleft = G_TimeTilExtremeSuddenDeath( );
    //warn about extreme sudden death when 5 minutes are left
    if( timeleft <= 300000 && level.extremeSuddenDeathWarning < TW_CLOSE )
    {
    ...
    if( timeleft <= 60000 && level.extremeSuddenDeathWarning < TW_IMMINENT )
    {
  ...
  {
    float alienPlayerCountMod = level.averageNumAlienClients / ALIEN_PLAYER_COUNT_MOD;
    float humanPlayerCountMod = level.averageNumHumanClients / HUMAN_PLAYER_COUNT_MOD;
    ...
    if( g_alienStage.integer == S1 && g_alienMaxStage.integer > S1 )
      alienNextStageThreshold = (int)( ceil( ( (float)g_alienStage2Threshold.integer * CREDIT_TO_FRAG_RATIO ) * alienPlayerCountMod ) );
    else if( g_alienStage.integer == S2 && g_alienMaxStage.integer > S2 )
      alienNextStageThreshold = (int)( ceil( ( (float)g_alienStage3Threshold.integer * CREDIT_TO_FRAG_RATIO ) * alienPlayerCountMod ) );
    else
...
void G_CalculateStages( void )
{
  ...
  // set alien stage 2, when they got enough "credits"
  if( g_alienCredits.integer >=
      (int)( ceil( (float)g_alienStage2Threshold.integer * alienPlayerCountMod * CREDIT_TO_FRAG_RATIO ) ) &&
  ...
  // same for alien stage 3
  if( g_alienCredits.integer >=
      (int)( ceil( (float)g_alienStage3Threshold.integer * alienPlayerCountMod * CREDIT_TO_FRAG_RATIO ) ) &&
  ...
  // set human stage 2, when they got enough credits
  ...
  if( g_alienStage.modificationCount > lastAlienStageModCount )
  {
    ...
    {
      level.alienStage2Time = level.time;
      G_LogPrintf( "A 3: Aliens reached Stage 3\n" );
    }
    else if( g_alienStage.integer == S3 )
    {
      level.alienStage3Time = level.time;
      G_LogPrintf( "A 2: Aliens reached Stage 3\n" );
    }
  ...
  if( g_humanStage.modificationCount > lastHumanStageModCount )
  {
    ...
    {
      level.humanStage2Time = level.time;
      G_LogPrintf( "H 2: Humans reached Stage 2\n" );
    }
    else if( g_humanStage.integer == S3 )
    {
      level.humanStage3Time = level.time;
      G_LogPrintf( "H 3: Humans reached Stage 3\n" );
    }
    lastHumanStageModCount = g_humanStage.modificationCount;
  }
...
void CheckExitRules( void )
{
  ...
  {
    if( level.numLiveHumanClients != 1 && level.numLiveAlienClients == 1 )
    {
      level.lastWin = TEAM_NONE;

      trap_SendServerCommand( -1, "print \"Alien Player Wins \n\"" );
      trap_SetConfigstring( CS_WINNER, "Alien Player Wins" );
      LogExit( "Alien Player Wins"  );
    ...
    {
      trap_SendServerCommand( -1, "print \"Human Player Wins \n\"" );
      trap_SetConfigstring( CS_WINNER, "Human Player Wins" );
      LogExit( "Human Player Wins" );
      G_admin_maplog_result( "t" ); //FIXME: Should have something special
    }
    else
    {
      trap_SendServerCommand( -1, "print \"No One Wins \n\"" );
      trap_SetConfigstring( CS_WINNER, "No One Wins" );
      LogExit( "No One Wins" );
      G_admin_maplog_result( "t" ); //FIXME: Should have something special
    }
  }
  ...
  //special tkmap stuff
  if( level.resetfbf && g_tkmap.integer )
  {
    trap_Cvar_Set( "g_friendlyBuildableFire", "0" );
    level.resetfbf = qfalse;
  }
...
void CheckVote( void )
{
  ...
  if( level.time - level.voteTime >= VOTE_TIME || ( level.voteYes + level.voteNo >= level.numVotingClients ) )
  {
    ...
    else if( voteYesPercent <= votePassThreshold  )
    {
      ...
      G_LogPrintf( "vote: Vote Failed (Y:%i N:%i, %i percent)\n", level.voteYes, level.voteNo, voteYesPercent );
