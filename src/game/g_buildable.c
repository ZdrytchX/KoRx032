...
void ASpawn_Die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod )
{
  ...
  new->fate = ( attacker && attacker->client && attacker->client->ps.stats[ STAT_TEAM ] == TEAM_ALIENS ) ? BF_TEAMKILLED : BF_DESTROYED;
  new->next = NULL;
  G_LogBuild( new );
    ...
    if( self->s.groundEntityNum )
    {
              ...
              self->s.origin2, BA_A_SPAWN, NULL ) ) != NULL )
      {
        ...
        else if( ent->s.number == ENTITYNUM_WORLD || ent->s.eType == ET_MOVER )
        {
          ...
          ent->client->ps.stats[ STAT_TEAM ] == TEAM_ALIENS && !ent->client->pers.paused )
        {
...
#define HOVEL_TRACE_DEPTH 60.0f // Aaron: slightly smaller than a tyrant, the tallest class
...
qboolean AHovel_Blocked( gentity_t *hovel, gentity_t *player, qboolean provideExit )
{
  ...
  if( provideExit )
  {
    ...
    player->r.contents = CONTENTS_SOLID;
...
void AHovel_Use( gentity_t *self, gentity_t *other, gentity_t *activator )
{
  ...
  if( self->spawned && G_FindOvermind( self ) )
  {
             ...
             activator->health > 0 && self->health > 0 )
    {
      ...
      // prevent team killing / killing
      activator->takedamage = qfalse;
      activator->r.contents = CONTENTS_CORPSE;
...
void AHovel_Die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod )
{
  ...
  if( self->active )
  {
    ...
    builder->takedamage = qtrue;
    builder->r.contents = CONTENTS_SOLID;
...
qboolean ATrapper_CheckTarget( gentity_t *self, gentity_t *target, int range )
{
  ...
  if( target->flags & FL_NOTARGET ) // Do they have notarget on?
    return qfalse;
  if( !G_Visible( self, target, CONTENTS_SOLID ) )  // Can we see them?
    return qfalse;
...
void HForceField_Shrink( gentity_t *self, qboolean shrink );
...
================
*/
void HForceField_Die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod )
{
//FIXME: Wire: buildlog needs this but it is buggy
  ...
  VectorCopy( self->s.origin2, new->origin2 );
  VectorCopy( self->s.angles2, new->angles2 );
  new->fate = ( attacker && attacker->client && attacker->client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS ) ? BF_TEAMKILLED : BF_DESTROYED;
  new->next = NULL;
  G_LogBuild( new );
...
Set shrink state for a forcefield. When unshrinking, checks to make sure there
is enough room.
================
*/
void HForceField_Shrink( gentity_t *self, qboolean shrink )
{
  ...
  if ( !shrink &&
    ...
    return;
  BG_BuildableBoundingBox( BA_H_FORCEFIELD, self->r.mins, self->r.maxs );
  if ( shrink )
  {
    self->r.maxs[ 2 ] = (int)( self->r.maxs[ 2 ] * FORCEFIELD_SHRINKPROP );
    self->shrunkTime = level.time;
  ...
  else
  {
    ...
    if ( self->spawned && self->health > 0 &&
         anim != BANIM_CONSTRUCT1 && anim != BANIM_CONSTRUCT2 )
    {
      G_SetIdleBuildableAnim( self, BG_Buildable( BA_H_FORCEFIELD )->idleAnim );
      G_SetBuildableAnim( self, BANIM_ATTACK2, qtrue );
    }
  }
              ...
              self->s.origin2, BA_H_SPAWN, NULL ) ) != NULL )
      {
        ...
        else if( ent->s.number == ENTITYNUM_WORLD || ent->s.eType == ET_MOVER )
        {
          ...
          return;
            ...
            ent->client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS && !ent->client->pers.paused )
        {
          //spawnblock protection
          if( self->spawnBlockTime && level.time - self->spawnBlockTime > 10000 && !ent->client->pers.paused )
          {
            //five seconds of countermeasures and we're still blocked
            //time for something more drastic
...
static gentity_t *G_Build( gentity_t *builder, buildable_t buildable, vec3_t origin, vec3_t angles )
{
  ...
  if( builder->client )
  {
    built->builtBy = G_admin_namelognum( builder->client->ps.clientNum );
  ...
  if( BG_Buildable( built->s.modelindex )->team == TEAM_ALIENS )
    built->powered = qtrue;
  ...
  if( builder->client ) 
  {
     ...
     if( builder->client->pers.teamSelection == TEAM_ALIENS ) 
     {
       level.alienStatsCounters.structsbuilt++;
     }
     else if( builder->client->pers.teamSelection == TEAM_HUMANS )
     {
