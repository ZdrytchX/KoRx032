    ...
    if( other->client )
    {
      if( other->client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS )
      {
        if( attacker->client && ( ( attacker->client->ps.weapon == WP_ABUILD2 
            && other->client->ps.stats[ STAT_CLASS ] != PCL_HUMAN_BSUIT ) 
...
void G_RunMissile( gentity_t *ent )
{
  ...
  // ignore interactions with the missile owner
   if( ent->s.time + 500 > level.time || ent->parent->s.eType == ET_BUILDABLE )    //wait half a second before we do anything, just in case it hasn't left our model yet.
  ...
  int damage, int radius, int speed )
{
  ...
  if( !BG_InventoryContainsUpgrade( UP_SURGE, self->client->ps.stats ) )
    ...
    thinkTime = (int) ( 10000 * ( 1.0f - (float)( damage/XAEL_SURGE_DMG_MOD ) / (float)(XAEL_TOTAL_CHARGE ) ) );
  ...
  if( ( damage == XAEL_DAMAGE && !BG_InventoryContainsUpgrade( UP_SURGE, self->client->ps.stats ) )
        || damage == (int)( XAEL_DAMAGE * XAEL_SURGE_DMG_MOD ) )
    bolt->nextthink = level.time;
  else if( damage != XAEL_SECONDARY_DAMAGE && damage != (int)( XAEL_SECONDARY_DAMAGE * XAEL_SURGE_DMG_MOD ) )
  ...
  int damage, int radius, int speed )
{
  ...
  if( ( damage == LCANNON_DAMAGE && !BG_InventoryContainsUpgrade( UP_SURGE, self->client->ps.stats ) )
        || damage == (int)( LCANNON_DAMAGE * LCANNON_SURGE_DMG_MOD ) )
    bolt->nextthink = level.time;
  else
    bolt->nextthink = level.time + 10000;
  ...
  if( !BG_InventoryContainsUpgrade( UP_SURGE, self->client->ps.stats ) )
    ...
    charge = (float)( damage - LCANNON_SECONDARY_DAMAGE ) 
              / ( LCANNON_DAMAGE * LCANNON_SURGE_DMG_MOD );
