...
g_admin_cmd_t g_admin_cmds[ ] = 
  {
    ...
    {"listadmins", G_admin_listadmins, "A",
      ...
      "(^5LEVEL^7) (^5name|start admin#^7)"
      "\n ^3Example:^7 '!listadmins <0' show all admins with lvl < 0"
      "\n ^3Example:^7 '!listadmins =5' show all admins with lvl 5"
      "\n ^3Example:^7 '!listadmins >7' show all admins with lvl > 7"
      "\n ^3Example:^7 '!listadmins =7 foo' show add lvl 7 admins with 'foo' in their name"
    },
    ...
    {"building", G_admin_building, "E",
      "show the builder of the building you are looking at",
      "",
    },
...
int G_admin_namelognum( int client )
{
    int namelogNum = -1, i;
    for( i = 0; ( i < MAX_ADMIN_NAMELOGS ) && &g_admin_namelog[ i ]; i++ )
    {
      if( g_admin_namelog[ i ]->slot == client )
      {
        namelogNum = i;
        break;
      }
...
static int admin_listadmins( gentity_t *ent, int start, char *search, int minlevel, int maxlevel )
{
  int drawn = 0;
  char guid_stub[9];
  char name[ MAX_NAME_LENGTH ] = {""};
  ...
  char lname_fmt[ MAX_NAME_LENGTH ] = {""};
  int i, j;
  gentity_t *vic;
  int l = 0;
  qboolean dup = qfalse;
  ...
  {
    if( ( g_admin_admins[ i ]->level >= minlevel ) && ( g_admin_admins[ i ]->level <= maxlevel ) ) 
   {
     if( search[ 0 ] )
     {
...
qboolean G_admin_setlevel( gentity_t *ent, int skiparg )
{
  ...
  if( matches > 1 )
  {
    ADMP( "^3!setlevel:^7 more than one match.  Use the admin number "
      "instead:\n" );
    admin_listadmins( ent, 0, name, 0, 1000000 );
    return qfalse;
  }
...
qboolean G_admin_kick( gentity_t *ent, int skiparg )
{
  ...
  AP( va( "print \"%s" S_COLOR_WHITE " has been kicked%s^7. reason: %s\n\"",
          vic->client->pers.netname,
  ...
  trap_DropClient( pids[ 0 ], DROP_NO_MESSAGE );
...
qboolean G_admin_ban( gentity_t *ent, int skiparg )
{
  ...
  AP( va( "print \"%s" S_COLOR_WHITE " has been banned by %s^7, duration: %s, reason: %s\n\"",
          g_admin_namelog[ logmatch ]->name[ 0 ],
          ( ent ) ? ent->client->pers.netname : "console",
          duration,
          ( *reason ) ? reason : "banned by admin" ) );
  ...
  trap_DropClient( g_admin_namelog[ logmatch ]->slot, DROP_NO_MESSAGE );
...
qboolean G_admin_adjustban( gentity_t *ent, int skiparg )
{
  ...
  AP( va( "print \"^3!adjustban: ^7ban #%d for %s^7 has been updated by %s^7 "
    ...
    ( *reason ) ? "reason: " : "",
    reason ) );
  // Aaron: I see no reason to change the banner
  //if( ent )
...
qboolean G_admin_listadmins( gentity_t *ent, int skiparg )
{
  ...
  int drawn = 0;
  int minlevel = 1;
  int maxlevel = 1000000; // should be high enough ;)
  if( G_SayArgc() > (skiparg+1) )
  {
    G_SayArgv( 1 + skiparg, s, sizeof( s ) );
    if( s[1] != '\0' )
    {
      qboolean negative = qfalse; // atoi can't handle negative numbers ...
      char *p = s;
      p++;
      skiparg++; //assmume we get something useful. if not we will decrement it again
      if( ( p[0] == '-' ) && ( p[1] != '\0' ) )
      {
        p++;
        negative = qtrue;
      ...
      {
        maxlevel = atoi(p);
        if( maxlevel >= 0 )    // no atoi error
        {
          if( negative )
            maxlevel = -maxlevel;
          maxlevel--; // user wants < x, so we compare with <= (x-1)
          minlevel = -1000000;
        }
        else
          maxlevel = 1000000; // reset it
      }
      else if( s[0] == '=' )
      {
        minlevel = atoi(p);
        if( minlevel >= 0 ) // no atoi error
        {
          if( negative )
            minlevel = -minlevel;
          maxlevel = minlevel;
      ...
      else if( s[0] == '>' )
      {
        ...
        if( minlevel >= 0 )
        {
          if( negative )
            minlevel = -minlevel;
          minlevel++; // user wants > x, so we compare with >= (x+1)
        }
        else
          minlevel = 1;
      }
      else
        skiparg--; //nothing useful.
    }
  }
  for( i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[ i ]; i++ )
  {
  ...
  if( !found )
  {
    ADMP( "^3!listadmins: ^7no admins with matching level found\n" );
    return qfalse;
  }
  ...
  drawn = admin_listadmins( ent, start, search, minlevel, maxlevel );
  ...
  if( search[ 0 ] )
  {
    ...
    {
      ADMP( va( "^3!listadmins:^7 found >20 admins matching '%s^7. Try a more narrow search.'\n",
        search ) );
    }
  }
  else
  {
    ADMBP_begin();
    ADMBP( va( "^3!listadmins:^7 showing admins %d - %d of %d.  ",
      ( found ) ? ( start + 1 ) : 0,
...
qboolean G_admin_admintest( gentity_t *ent, int skiparg )
{
  ...
  if( !G_admin_permission( ent, ADMF_PERMMUTED ) && !ent->client->pers.muted )
  {
    ...
    AP( "print \"^3!admintest: You are not allowed to use this command as you are muted\n\"" );
  return qtrue;
}
...
qboolean G_admin_spec999( gentity_t *ent, int skiparg )
{
  ...
  for( i = 0; i < level.maxclients; i++ )
  {
    ...
    {
      // if they're alive refund their credits
      if( vic->health > 0 )
      {
        G_AddCreditToClient( vic->client, 
          BG_GetValueOfPlayer( &vic->client->ps ), qfalse );
      }
...
qboolean G_admin_restart( gentity_t *ent, int skiparg )
{
  ...
  AP( va( "print \"^3!restart: ^7map restarted by %s^7 %s %s\n\"",
          ( ent ) ? ent->client->pers.netname : "console",
          ( layout[ 0 ] ) ? va( "(forcing layout '%s')", layout ) : "",
          teampref ) );
  return qtrue;
...
qboolean G_admin_putmespec( gentity_t *ent, int skiparg )
{
  ...
  // muted people shouldn't make noise
  if( !G_admin_permission( ent, ADMF_PERMMUTED ) && !ent->client->pers.muted )
...
qboolean G_admin_warn( gentity_t *ent, int skiparg )
{
  ...
  AP( va( "print \"^3!warn: ^7%s^7 has been warned to cease and desist %s^7 by %s \n\"",
...
{ //this is all very similar to denybuild, 
  ...
  int i, found, pids[ MAX_CLIENTS ], pid;
  ...
  {
    G_SayArgv( 1 + skiparg, name, sizeof( name ) );
    found = G_ClientNumbersFromString( name, pids, MAX_CLIENTS );
    if( found != 1 )
    {
      G_MatchOnePlayer( pids, found, err, sizeof( err ) );
      ADMP( va( "^3!%s: ^7%s\n", cmd, err ) );
  ...
  for( i = 0; (i < MAX_CLIENTS) && (!targeted || (i < found)); i++ )
  {
    if( targeted )
      pid = pids[ i ];
    else
    ...
    if( !vic || !vic->client || vic->client->pers.connected != CON_CONNECTED ) 
      continue;
    //Com_Printf("(un)pausing entnum %i:\n", pids[ i ]);
    if( !admin_higher( ent, vic ) )
    {
      if( targeted )
      {
    ...
    if( vic->client->pers.paused )
    {
      ...
      CPx( pid, "cp \"^2You've been unpaused\"" );
      if( targeted )
    ...
    else
    {
      ...
      CPx( pid, va( "cp \"^1You've been paused by ^7%s\"", 
	  ( ent ) ? ent->client->pers.netname : "console" ) );
      if( targeted )
...
qboolean G_admin_building( gentity_t *ent, int skiparg )
{
  vec3_t      forward, end;
  trace_t     tr;
  gentity_t   *traceEnt;
  char        arg[8];
  char        state = -1;// -1 = error, 0 = info, 1 = revert, 2 = force revert
  char        *name, *article, *bname;
  ...
  if( state == -1 )
  {
    ADMP( "^3!building: ^7usage: !building (^5-r^7 (^5!^7))\n" );
    return qfalse;
  }
  ...
  VectorMA( ent->client->ps.origin, 8192 * 32, forward, end );
  ...
  {
    ADMP( "^3!building: ^7no building in view.\n" );
    return qfalse;
  }
  
  name = NULL; // any non-client action
  if( traceEnt->builtBy >= 0 ) {
    for( i = (MAX_ADMIN_NAMELOG_NAMES-1); i >= 0; i-- )
    {
      if( g_admin_namelog[ traceEnt->builtBy ]->name[ i ][ 0 ] )
        break;
    }
    name = g_admin_namelog[ traceEnt->builtBy ]->name[i];
  }
  
  if( state == 0 )
  {
    ADMP( va( "^3!building: ^7This %s was built by %s\n",
        BG_FindHumanNameForBuildable( traceEnt->s.modelindex ),
        name ? va ("'%s^7'", name ) : "<world>" ) );
    return qtrue;
  }
  
  if( (traceEnt->s.modelindex == BA_A_SPAWN )
      && (level.numAlienSpawns <= 1) && ( state == 1) )
  {
    ADMP( "^3!building: ^7This would remove the ^1Aliens^7 last spawn, use ^3!building ^5-r !^7 to ignore this.\n" );
    return qfalse;
  }
  else if ( (traceEnt->s.modelindex == BA_H_SPAWN )
      && (level.numHumanSpawns <= 1) && ( state == 1) )
  {
    ADMP( "^3!building: ^7This would remove the ^4Humans^7 last spawn, use ^3!building ^5-r !^7 to ignore this.\n" );
    return qfalse;
  }
  
  bname = BG_FindHumanNameForBuildable( traceEnt->s.modelindex );
  // this is similar to the buildlog stuff
  if( BG_FindUniqueTestForBuildable( traceEnt->s.modelindex ) )
    article = "the";
  else if( strchr( "aeiouAEIOU", *bname ) )
    article = "an";
  else
    article = "a";
  AP( va( "print \"%s^7 reverted %s^7'%s building of %s %s\n\"",
      ent->client->pers.netname,
      name ? name : "<world>",
      (name && strchr( "Ss", name[ strlen( name ) - 1 ] )) ? "" : "s",
      article, bname ) ); 
  
  ptr = prev = level.buildHistory;
  while( ptr )
  {
    VectorSubtract( traceEnt->s.pos.trBase, ptr->origin, dist );
    if( VectorLength( dist ) > 10 ) {
      // number is somewhat arbitrary, watch for false pos/neg
      prev = ptr;
      ptr = ptr->next;
      continue;
    }
    ...
    if( ptr == level.buildHistory )
      level.buildHistory = ptr = ptr->next;
    else
  ...
  for( i = 0, ptr = prev = level.buildHistory; repeat > 0; repeat--, j = 0 )
  {
    ...
    switch( ptr->fate )
    {
      case BF_BUILT:
        ...
        j = MAX_CLIENTS;
        targ = g_entities + MAX_CLIENTS;
        for( ; j < level.num_entities; j++, targ++ )
        {
...
qboolean G_admin_drop( gentity_t *ent, int skiparg )
{
  ...
  //what people get
  trap_DropClient( pids[ 0 ], DROP_DISCONNECT );
...
qboolean G_admin_credits( gentity_t *ent, int skiparg )
{
  ...
  credits = vic->client->pers.Credit;
   
  if( vic->client->ps.stats[ STAT_TEAM ] == TEAM_ALIENS )
    amnt_f *= ALIEN_CREDITS_PER_FRAG;
  amnt = (int) amnt_f;
  ...
  switch( amnt_chr[0] )
  {
    case '+':
      ...
      AP( va( "print \"^3!credits: ^7%s^7 gave ^7%s^7 %s\n\"", ( ent ) ? ent->client->pers.netname : "console", vic->client->pers.netname, G_ReadableCredits( amnt, vic->client->ps.stats[ STAT_TEAM ] ) ) );
      break;
    ...
    case '-':
      ...
      AP( va( "print \"^3!credits: ^7%s^7 subtracted %s from ^7%s^7\n\"", ( ent ) ? ent->client->pers.netname : "console", G_ReadableCredits( amnt, vic->client->ps.stats[ STAT_TEAM ] ), vic->client->pers.netname ) );
      break;
  }
  return qtrue;
}
