...
==================
*/
int G_RewardAttackers( gentity_t *self )
{
  int team, i, value, maxHealth = 0, totalDamage = 0;
  ...
  for( i = 0; i < MAX_CLIENTS; i++ )
  {
    ...
    short num = stageValue;
    if( totalDamage < maxHealth )
      stageValue = ( value * self->credits[ i ] ) / maxHealth;
    else
      stageValue = ( value * self->credits[ i ] ) / totalDamage;
    ...
    //else
    //  G_AddCreditToClient( player->client, num*( HUMAN_MAX_CREDITS/ALIEN_MAX_CREDITS ), qtrue );
...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath )
{
  ...
  if( meansOfDeath == MOD_KILL )
    goto finish_dying;
  
  if( !tk || g_tkmap.integer || meansOfDeath == MOD_TELEFRAG || meansOfDeath == MOD_SLAP )
  {
  ...
  else if( self->client->pers.teamSelection == TEAM_HUMANS )
  {
  ...
  if( attacker && attacker->client )
  {
    ...
    if( ( ( g_devmapKillerHP.integer && g_cheats.integer ) || g_KillerHP.integer ) && attacker != self && !OnSameTeam( self, attacker ) )
    {
      trap_SendServerCommand( self-g_entities, 
        attacker->health > 0 ? va( "print \"Your killer, %s^7, had ^2%3i ^7HP.\n\"", killerName, attacker->health ) 
                             : va( "print \"Your killer, %s^7, died before he killed you.\n\"", killerName ) );
    }
    ...
    for( i = 0; i < MAX_CLIENTS; i++ )
    {
      ...
      totalTK += self->client->tkcredits[ i ];
    }
    ...
    if( g_retribution.integer && totalTK && !g_tkmap.integer )
    {
      ...
      {
        totalPrice = BG_ClassCanEvolveFromTo( PCL_ALIEN_LEVEL0, self->client->ps.stats[ STAT_CLASS ], self->client->pers.Credit, g_alienStage.integer, 0, 0 );
        max = ALIEN_MAX_CREDITS;
      }
      else
        totalPrice = BG_GetValueOfEquipment( &self->client->ps );

      if( self->client->pers.Credit + totalPrice > max )
        totalPrice = max - self->client->pers.Credit;

      if( totalPrice > 0 )
      {
        int recievedCredits = 0;
        totalTK += totalDamage;
        ...
        for( i = 0; i < MAX_CLIENTS; i++ )
        {
          ...
          owed = price = ( totalPrice * self->client->tkcredits[ i ] ) / totalTK;
          self->client->tkcredits[ i ] = 0;
          
          ourTkCredits = g_entities[ i ].client->tkcredits + self->client->ps.clientNum;
          // subtract what they owe us
          if( *ourTkCredits > 0 )
          {
            if( price - *ourTkCredits < 0 )
            {
              *ourTkCredits -= price;
              price = 0;
            }
          ...
          if( g_entities[ i ].client->pers.Credit < price )
            price = g_entities[ i ].client->pers.Credit;
          if( price )
          {
            team_t team = self->client->pers.teamSelection;
            // take it away
            g_entities[ i ].client->pers.Credit -= price;
            // add it
            recievedCredits += price;
            //tell them
            trap_SendServerCommand( self->client->ps.clientNum,
                va( "print \"Received ^3%s of ^3%s owed from %s ^7in retribution.\n\"",     
                    G_ReadableCredits( price, team ),
                    G_ReadableCredits( owed, team ),
                    g_entities[ i ].client->pers.netname ) );
            trap_SendServerCommand( g_entities[ i ].client->ps.clientNum,
                va( "print \"Transfered ^3%s of ^3%s owed to %s ^7in retribution.\n\"",
                    G_ReadableCredits( price, team ),
                    G_ReadableCredits( owed, team ),
        ...
        // give it to us
        self->client->pers.Credit += recievedCredits;
      }
    }
    else
    {
...
static float G_CalcDamageModifier( vec3_t point, gentity_t *targ, gentity_t *attacker, int class, int dflags )
{
  ...
  if( attacker && attacker->client && modifier == 2 )
  {
    ...
    // crouches take some damage off headshots
         ...
         vec3_t dir, vec3_t point, int damage, int dflags, int mod )
{
  ...
  if( targ->s.eType == ET_MOVER )
  {
    ...
    if( attacker->client->pers.teamSelection == TEAM_ALIENS 
        && !( mod == MOD_LEVEL2_ZAP || mod == MOD_POISON || mod == MOD_INFECTION || mod == MOD_SPITFIRE_ZAP ) )
    {
  ...
  {
    // special case for adv rant eblob, goon barb, and spitbomb not doing self damage
    if( targ == attacker &&
    ...
    if( targ != attacker && OnSameTeam( targ, attacker ) )
    {
      // don't do friendly fire on movement attacks
      if( ( mod == MOD_LEVEL4_TRAMPLE || mod == MOD_LEVEL3_POUNCE ||
          mod == MOD_LEVEL4_CRUSH ) && !g_friendlyFireMovementAttacks.integer
          && !g_tkmap.integer )
        return;
  ...
  if( client )
  {
    ...
    {
      if( mod != MOD_POISON && mod != MOD_LEVEL2_ZAP && mod != MOD_INFECTION
          && mod != MOD_SPITFIRE_ZAP &&
  ...
  if( take )
  {
    ...
    {
      int contents;
      contents = trap_PointContents( attacker->s.origin, -1 );
      if( level.vesd && attacker->health > 0 
          && attacker->client->ps.stats[ STAT_TEAM ] != TEAM_NONE )
      {
        int health = attacker->health;
        //if( contents & CONTENTS_VAMPIREZONE )
        //FIXME: Camper added a already used bitmask for CONTENTS_VAMPIREZONE
        //       I'm editing this instead of redoing the qcommon and rereleasing 0.3.2
        health += ( 0.5 * takeNoOverkill * attacker->client->ps.stats[ STAT_MAX_HEALTH ] ) / targ->client->ps.stats[ STAT_MAX_HEALTH ];
        //else
        //  health += ( takeNoOverkill * attacker->client->ps.stats[ STAT_MAX_HEALTH ] ) / targ->client->ps.stats[ STAT_MAX_HEALTH ];
        if( health > MAX_HEALTH )
          health = MAX_HEALTH;
        attacker->client->ps.stats[ STAT_HEALTH ] = attacker->health = health;
      }
...
void G_LogDestruction( gentity_t *self, gentity_t *actor, int mod )
{
  ...
  if( self->health <= 0 && mod == MOD_DECONSTRUCT )
    dead = qtrue;
  ...
  if( mod == MOD_DECONSTRUCT )
    G_LogPrintf( "Decon: %d %d %d: %s^7 deconstructed %s %s\n",
      actor->client->ps.clientNum,
      self->s.modelindex,
      mod,
      actor->client->pers.netname,
      BG_Buildable( self->s.modelindex )->name,
      dead ? "(building was dead)" : "" );
  else
