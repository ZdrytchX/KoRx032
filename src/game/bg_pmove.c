...
float pm_flyaccelerate = 2.75f;

float pm_friction = 6.0f;
float pm_waterfriction = 1.0f;
float pm_flightfriction = 1.25f;
float pm_spitfriction = 1.25f;
...
static float PM_CmdScale( usercmd_t *cmd )
{
  ...
  if( ( pm->ps->weapon == WP_ALEVEL4 ||
        pm->ps->weapon == WP_ALEVEL4_UPG ) && pm->ps->pm_flags & PMF_CHARGE )
...
static qboolean PM_CheckPounce( void )
{
  ...
  {
    if( ( pm->ps->pm_flags & PMF_CHARGE ) ||
        pm->ps->stats[ STAT_MISC ] < LEVEL0_DRILL_TIME_MIN ||
        pm->ps->groundEntityNum == ENTITYNUM_NONE )
      return qfalse;
  }
  ...
  else if( pm->ps->weapon == WP_ALEVEL3_UPG )
    jumpMagnitude = pm->ps->stats[ STAT_MISC ] *
  ...
  if( pm->ps->stats[ STAT_STATE ] & SS_SLOWLOCKED )
    jumpMagnitude *= ABUILDER_BLOB_SPEED_MOD;
...
static qboolean PM_CheckJump( void )
{
  ...
  if( pm->ps->pm_type == PM_GRABBED
      && !( pm->ps->stats[ STAT_STATE ] & SS_BLOBLOCKED ) )
  {
    pm->cmd.upmove = 0;
    return qfalse;
  }
  ...
  if( !( pm->ps->stats[ STAT_STATE ] & SS_SLOWLOCKED ) )
    ...
    VectorMA( pm->ps->velocity, BG_Class( pm->ps->stats[ STAT_CLASS ] )->jumpMagnitude * ABUILDER_BLOB_SPEED_MOD,
              normal, pm->ps->velocity );
...
static void PM_GroundClimbTrace( void )
{
  ...
  for( i = 0; i <= 4; i++ )
  {
    ...
    if( trace.fraction < 1.0f && !( trace.surfaceFlags & ( SURF_SKY | SURF_SLICK ) ) && 
      ...
      !( trace.entityNum >= MAX_GENTITIES && i != 4 ) )
#else
...
static void PM_Weapon( void )
{
  ...
  if( pm->ps->weapon == WP_XAEL )
  {
        ...
        ( pm->cmd.buttons & BUTTON_ATTACK2 ) )
    {
      ...
      if( pm->ps->stats[ STAT_MISC ] >= XAEL_CHARGE_TIME_MAX )
        pm->ps->stats[ STAT_MISC ] = XAEL_CHARGE_TIME_MAX;
      if( pm->ps->stats[ STAT_MISC ] > pm->ps->ammo * XAEL_CHARGE_TIME_MAX /
                                              XAEL_CHARGE_AMMO )
        pm->ps->stats[ STAT_MISC ] = pm->ps->ammo * XAEL_CHARGE_TIME_MAX /
                                            XAEL_CHARGE_AMMO;
    }

    // Set overcharging flag so other players can hear the warning beep
    if( pm->ps->stats[ STAT_MISC ] > XAEL_CHARGE_TIME_WARN )
      pm->ps->eFlags |= EF_WARN_CHARGE;
  }
  ...
  if( !BG_Weapon( pm->ps->weapon )->usesEnergy &&
          ( BG_InventoryContainsUpgrade( UP_AMMOPACK, pm->ps->stats ) ||
          BG_InventoryContainsUpgrade( UP_BATTLESUIT, pm->ps->stats ) ) )
  ...
  else if( attack2 )
  {
    ...
    {
      if( pm->ps->weapon == WP_SHOTGUN && pm->ps->ammo < SHOTGUN_NADE_TAKE )
      {
        PM_AddEvent( EV_NOAMMO );
        pm->ps->weaponTime += 200;
        return;
          ...
          pm->ps->weapon == WP_SPITFIRE ) && attack3 ) )
  {
    ...
    }//special case for shotgun grenades
    else if( pm->ps->weapon == WP_SHOTGUN && attack2 )
    {
      pm->ps->ammo -= SHOTGUN_NADE_TAKE;
    }
    else
    {
      pm->ps->ammo--;
