...
void buildFire( gentity_t *ent, dynMenu_t menu )
{
  ...
  {
    if( ent->client->ps.stats[ STAT_MISC ] > 0 && !ent->client->pers.override )
    {
  ...
  for( i = 0; i < num; i++ )
  {
        ...
        humanPlayer->client->pers.teamSelection == TEAM_HUMANS )
    {
          ...
          && !BG_InventoryContainsUpgrade( UP_HELMET, humanPlayer->client->ps.stats ) )
      {
        ...
        if( ent->client && ent->client->ps.stats[ STAT_STATE ] & SS_BOOSTED
            && humanPlayer->client->poisonImmunityTime < level.time )
        {
          //copied from g_combat
    ...
    if( enemy->client && enemy->client->ps.stats[ STAT_HEALTH ] > 0 )
    {
      ...
      if( BG_InventoryContainsUpgrade( UP_JETPACK, enemy->client->ps.stats ) )
      {
        ...
        if( enemy->client->ps.stats[ STAT_JET_CHARGE ] > 0
            && ent->client->ps.stats[ STAT_CLASS ] != PCL_ALIEN_SPITFIRE
            && rand( ) % 2 )
          enemy->client->ps.stats[ STAT_JET_CHARGE ]--;
        
        //once in a blue moon... BOOM!
        if( rand() % chance  == (int)( chance / 2 ) )
        {
          gentity_t *tent;
          ...
          if( enemy->client->ps.stats[ STAT_HEALTH ] > 0 )
          {
            ...
            G_RadiusDamage( enemy->s.origin, enemy, SHOTGUN_NADE_DAMAGE*JETPACK_EXPLODE_MOD,
...
qboolean CheckPounceAttack( gentity_t *ent )
{
  ...
  if( ent->client->ps.weapon == WP_ALEVEL3_UPG )
    damage = payload * LEVEL3_POUNCE_UPG_DMG / timeMax;
  else
  ...
  switch( ent->s.weapon )
  {
    ...
    case WP_ALEVEL3_UPG:
      meleeAttack( ent, LEVEL3_CLAW_UPG_RANGE, LEVEL3_CLAW_WIDTH,
                   LEVEL3_CLAW_WIDTH, LEVEL3_CLAW_UPG_DMG, MOD_LEVEL3_CLAW );
      break;
    case WP_ALEVEL2:
