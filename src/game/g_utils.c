...
char *G_ReadableCredits( int credits, team_t team )
{
  if( team == TEAM_ALIENS )
  {
    float evos;
    evos = ( (float) credits ) / ( (float) ALIEN_CREDITS_PER_FRAG );
    
    if( evos == 0.0 )
      return va( "^3nothing^7" );
    else if( ( evos > 0.0 ) && ( evos < 0.1f ) )
      return va( "<0.1 ^7evos" );
    else {
      ...
      // workaround for fracpart = modf( evos, &intpart);
      intpart = (float) ( (int) evos );
      fracpart = evos - intpart;
      
      if( fracpart >= 0.95 )
        evos = intpart + 1.0;
      else if( fracpart < 0.05 )
        evos = intpart;
      else
        precision = 1;
      
      return va( "%s%.*f ^7evo%s", negative ? "-" : "", precision, evos, (evos == 1.0) ? "" : "s" );
    }
  }
  else
    return va( "%i ^7credit%s", credits, (credits == 1) ? "" : "s" );
}
