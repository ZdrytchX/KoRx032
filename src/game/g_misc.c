...
======================================================================
  ...
  NEAT EFFECTS AND STUFF FOR TREMFUSION
...
distribute credits to the players specified by recipients,
store who recivieved how mutch and return the amount of credits,
that where given out.

credits
  How many credits to distribute
recipients[]
  Array of recipients; terminated by -1.
  This array will be modified!
  All players must be alive and in a team!
max_credits
  If the player has more than this value he woun't get any credits.
distributed[]
  If not NULL it's length has to be MAX_CLIENTS and the amount of
  crdits given out to each player will be stored here.
  The array will not be initialized!

return value
  Total amount of credits given out.

================
*/
short distributeCredits( short credits, CLIENTNUM_TYPE recipients[],
                         short max_credits, short distributed[] )
{
  short i, n, num_recipients, value, skip, portion, thisportion;
  gclient_t *cl;
  
  for( num_recipients = 0; recipients[num_recipients] >= 0; num_recipients++ );
  
  value = credits;
  
  while( ( value > 0 ) && ( num_recipients > 0 ))
  {
    portion = value / num_recipients;
    if( portion < 1 ) // if the remaining value is less than the number of remaining players
      portion = 1;    // the portion will be 0. this makes it possible to leave the loop in the above case.
    
    for( i = skip = 0; ( i < num_recipients ) && ( value > 0 ); i++ )
    {
      if( portion > value )  // also never give more out then we have.
        portion = value;     // this happens if the portion was 1.
      
      n = recipients[ i ];
      cl = &level.clients[ n ];
      
      if( cl->pers.Credit + portion >= max_credits )
      {
        thisportion = max_credits - cl->pers.Credit; // don't give him more then he needs
        skip++;                                           // to get MAX_CREDITS
        // if the player has enough credits, we don't need to shift him in the array,
        // because his slot will be overwriten.
      }
      else 
      {
        thisportion = portion;
        recipients[ i-skip ] = n; // shift the current player to the left.
      }
      value -= thisportion;
      cl->pers.Credit += thisportion; // give it to the player
      if( distributed )                    // if distributed is given,        
        distributed[ n ] += thisportion;   //   store the add the value here
    }
    num_recipients -= skip; // reduce the "length" of the array
  }
