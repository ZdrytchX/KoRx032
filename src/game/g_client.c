...
{
  int       i, overflow = 0, max = 0, overflowed = 0, overflowtotal = 0;
  gclient_t *cl;
  ...
  if( client->pers.Credit + credit > max 
      && !g_extremeSuddenDeath.integer && !g_cheats.integer
      && g_allowShare.integer && cap && !client->pers.override )
  {
    ...
    for( i = 0; overflow > 0 && i < MAX_CLIENTS; i++ )
    {
     ...
     thisteam = cl->ps.stats[ STAT_TEAM ];
      ...
      else
      {
        ...
        cl->pers.Credit += overflowamt;
        overflow = 0;
      }

      trap_SendServerCommand( k,
      va( "print \"%s^7 overflowed ^2%s to you!\n\"",
      client->pers.netname, G_ReadableCredits( overflowamt, thisteam ) ) );
    }
    ...
    for( i = 0; overflow > 0 && i < MAX_CLIENTS; i++ )
    {
      ...
      trap_SendServerCommand( k,
      va( "print \"%s^7 overflowed ^2%s ^7to you!\n\"",
      client->pers.netname, G_ReadableCredits( overflowamt, thisteam ) ) );
    }
    ...
    {      
      trap_SendServerCommand( client - level.clients,
       va( "print \"^7You overflowed ^2%s ^7to ^2%d ^7%s\n\"",
       G_ReadableCredits( overflowtotal, team ), overflowed, ( overflowed == 1 ) ? "person" : "people" ) );
    }
  }
  else if( client->pers.Credit + credit > max && cap )
  {
...
void ClientUserinfoChanged( int clientNum )
{
  ...
  char      adminname[ MAX_INFO_STRING ];
  ...
  if( !strstr( ent->client->pers.netname, g_adminstring.string )
    ...
    strcpy( adminname, va( "%s%s", g_adminstring.string, S_COLOR_WHITE ) );
  else
    strcpy( adminname, "" );
  ...
  if( strcmp( oldname, newname ) )
  {
    ...
    G_ClientCleanName( va( "%s", client->pers.netname ), oldname, sizeof( oldname ) );
    ...
    else if( strstr( newname, g_adminstring.string )
             && !G_admin_permission( ent, ADMF_ADMINCHAT ) )
    {
      trap_SendServerCommand( ent - g_entities,
        va( "print \"%s is a character reserved to denote admins.\n\"",
            g_adminstring.string ) );
      revertName = qtrue;
    }
  ...
  Com_sprintf( userinfo, sizeof( userinfo ),
    "n\\%s%s\\t\\%i\\model\\%s\\c1\\%s\\c2\\%s\\"
    "hc\\%i\\ig\\%16s\\v\\%s", adminname,
  ...
  if( client->pers.guid[0] )
  {
    ...
    for( i = 0; i < level.maxclients; i++ )
    {
      ...
      if( !Q_stricmp( client->pers.guid, level.clients[ i ].pers.guid ) )
      {
        ...
        trap_SendServerCommand( -1, va( "print \"%s" S_COLOR_WHITE " Ghost\n\"", client->pers.netname ) );
        trap_DropClient( i, DROP_NO_MESSAGE );
      }
    }
  }
  else
...
#define F_VEL   50.0f
      ...
      client->ps.stats[ STAT_TEAM ] == TEAM_ALIENS )
  {
    ...
    else
    {
      ...
      // FIXME: This bleeds through solid objects
           ...
           client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS )
  {
    spawn_angles[ YAW ] += 180.0f;
    AngleNormalize360( spawn_angles[ YAW ] );
    // FIXME: This bleeds through solids
    //G_AddPredictableEvent( ent, EV_PLAYER_RESPAWN, 0 );
...
============
*/
void ClientDisconnect( int clientNum, int message )
{
  gentity_t *ent;
  gentity_t *tent;
  int       i;
  ...
  switch( message )
  {
    case DROP_DISCONNECT:
      reason = "disconnected";
      break;
    case DROP_COMMAND_OVERFLOW:
      reason = "Server command overflow";
      break;
    case DROP_INACTIVITY:
      reason = "was dropped due to inactivity";
      break;
    case DROP_TIMEOUT:
      reason = "timed out";
  }
