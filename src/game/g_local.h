...
typedef struct
{
  ...
  int                 reportCount;        // because otherwise the file's not worth reading
...
typedef struct
{
  ...
  qboolean          extremeSuddenDeathVote;
...
#define CMD_DEAD          0x0100
#define CMD_INTERMISSION  0x0200 // valid during intermission
...
void        G_EscapeString( char *out, char *in, int maxlen );
...
short distributeCredits( short credits, CLIENTNUM_TYPE recipients[], short max_credits, short distributed[] );
...
extern  vmCvar_t  g_reportFile;
extern  vmCvar_t  g_reportLimit;
...
extern  vmCvar_t  g_noguidnobuild;

extern  vmCvar_t  g_adminstring;


extern  vmCvar_t  g_randommappercent;
...
void      trap_DropClient( int clientNum, int message );
