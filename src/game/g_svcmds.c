  ...
  if( atoi( name ) == -1 )
  {
    ...
    for( i = 0; i < level.maxclients; i++ )
    {
      ...
      if( level.clients[ i ].pers.localClient )
        continue;
      trap_SendServerCommand( i, va( "disconnect \"%s\n\"", reason ) );
      trap_DropClient( i, DROP_DISCONNECT );
    }
  }
  else
  {
    ...
    trap_SendServerCommand( cl-level.clients, va( "disconnect \"%s\n\"", reason ) );
    trap_DropClient( cl-level.clients, DROP_DISCONNECT );
  }
}
...
// dumb wrapper for "a", "m", "c", and "say"
