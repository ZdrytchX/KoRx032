...
void Cmd_Team_f( gentity_t *ent )
{
  ...
  if( !Q_stricmp( s, "auto" ) )
  {
    ...
    if( ( level.humanTeamLocked || humans > aliens ) 
               && !level.alienTeamLocked )
      team = TEAM_ALIENS;
    else if( ( level.alienTeamLocked || aliens > humans )
               && !level.humanTeamLocked )
      team = TEAM_HUMANS;
    else if( !level.alienTeamLocked || !level.humanTeamLocked )
  ...
  if( oldteam != TEAM_NONE && ent->client->sess.spectatorState == SPECTATOR_NOT &&
     ( ent->client->ps.stats[ STAT_CLASS ] == PCL_ALIEN_BUILDER0 ||
      ...
      ent->client->ps.stats[ STAT_MISC ] > 0 && !ent->client->pers.override )
  {
    if( ent->client->pers.teamSelection == TEAM_ALIENS )
...
void G_Say( gentity_t *ent, gentity_t *target, int mode, const char *chatText )
{
  ...
  char        location[ 64 ], admin[ MAX_NAME_LENGTH ];
  ...
  if( ent && ent->client )
  {
    ...
    if( !strstr( ent->client->pers.netname, g_adminstring.string )
        && G_admin_permission( ent, ADMF_ADMINCHAT )
        && !G_admin_permission( ent, ADMF_INCOGNITO ) )
      strcpy( admin, va( "%s" S_COLOR_WHITE, g_adminstring.string ) );
  }
  else
  {
    prefix = "";
  ...
  else if( level.extremeSuddenDeath && !level.intermissiontime 
           && mode != SAY_TEAM && ent 
           && ent->client->pers.teamSelection == TEAM_NONE 
           && !G_admin_permission( ent, ADMF_NOCENSORFLOOD  ) )
  {
    trap_SendServerCommand( ent-g_entities, va( "print \"Global chatting for "
      "spectators is disabled during ESD.\n\"") );
    return;
  }
  ...
  switch( mode )
  {
    ...
    case SAY_ALL:
      ...
      Com_sprintf( name, sizeof( name ), "%s%s%s" S_COLOR_WHITE ": ", prefix, admin,
    ...
    case SAY_TEAM:
      ...
      if( Team_GetLocationMsg( ent, location, sizeof( location ) ) )
        Com_sprintf( name, sizeof( name ), "(%s%s" S_COLOR_WHITE ") (%s): ", admin,
          ( ent ) ? ent->client->pers.netname : "console", location );
      else
        Com_sprintf( name, sizeof( name ), "(%s%s" S_COLOR_WHITE "): ", admin,
          ( ent ) ? ent->client->pers.netname : "console" );
      color = COLOR_CYAN;
    ...
    case SAY_ACTION:
      ...
      Com_sprintf( name, sizeof( name ), "^2%s^7%s%s%s%c%c ", g_actionPrefix.string, prefix, admin,
    ...
    case SAY_ACTION_T:
      ...
      if( Team_GetLocationMsg( ent, location, sizeof( location ) ) )
        Com_sprintf( name, sizeof( name ), EC"^5%s^7%s%s%c%c (%s) ", g_actionPrefix.string, admin,
          ( ent ) ? ent->client->pers.netname : "console", Q_COLOR_ESCAPE, COLOR_WHITE, location );
      else
        Com_sprintf( name, sizeof( name ), EC"^5%s^7%s%s%c%c ", g_actionPrefix.string, admin,
          ( ent ) ? ent->client->pers.netname : "console", Q_COLOR_ESCAPE, COLOR_WHITE );
      color = COLOR_WHITE;
      break;
...
void Cmd_CallVote_f( gentity_t *ent )
{
  ...
  if( ptr )
  {
    *ptr = '\0';
    ...
    if( ( *ptr == 'r' || *ptr == 'R' ) && ( *(ptr+1) == ' ' ) )
    {
      ptr+=2;
      G_EscapeString( reason, ptr, sizeof(reason) );
    }
    else if( ( ( *ptr == 'c' || *ptr == 'C' ) && ( *(ptr+1) == ' ' ) ) && !Q_stricmp( arg1, "poll" ) )
    {
      ptr+=2;
      level.votePassThreshold = atoi( ptr );
      if( level.votePassThreshold > 99 )
        level.votePassThreshold = 99;
      else if( level.votePassThreshold < 0 ) // -> -1 = error
        level.votePassThreshold = 50;
      for( ; *ptr >= '0' && *ptr <= '9' ; ptr++ ); // skip numbers
      for( ; *ptr == ' '; ptr++ ); // skip whitespace
      arg2plus = ptr; // and set arg2plus correctly
    }
    else
    {
  ...
  if( ( !Q_stricmp( arg1, "map" ) || !Q_stricmp( arg1, "nextmap" ) )
			&& strchr( arg2, '.' ) )
	{
    trap_SendServerCommand( ent-g_entities, "print \"Maps can not have punctuation in their titles\n\"" );
		return;
	}
    ...
    if( G_admin_permission( &g_entities[ clientNum ], ADMF_IMMUNITY ) )
    {
      ...
      //return;
			Com_sprintf( level.voteString, sizeof( level.voteString ),
				"m %d a kick vote against you passed. it was called by ^7%s", clientNum,
  ...
  else if( !Q_stricmp( arg1, "mute" ) )
  {
    ...
    if( G_admin_permission( &g_entities[ clientNum ], ADMF_IMMUNITY ) )
    {
      ...
      // return;
      Com_sprintf( level.voteString, sizeof( level.voteString ),
        "m %d a mute vote against you passed. it was called by ^7%s", clientNum,
      ent->client->pers.netname );
    }
    else
    {
  ...
  else if( !Q_stricmp( arg1, "poll" ) )
  {
    ...
    G_EscapeString( message, arg2plus, sizeof(message) );
    Com_sprintf( level.voteString, sizeof( level.voteString ), nullstring );
    Com_sprintf( level.voteDisplayString,
        sizeof( level.voteDisplayString ), "[Poll] \'%s\'", message );
  }
  ...
  else if( !Q_stricmp( arg1, "forcespec" ) )
  {
    ...
    {
      Com_sprintf( level.voteString, sizeof( level.voteString ),
        "m %d a forcespec vote against you passed. it was called by ^7%s", clientNum,
      ent->client->pers.netname );
    }
    else
    {
  ...
  else if( !Q_stricmp( arg1, "unforcespec" ) )
  {
    ...
    {
      Com_sprintf( level.voteString, sizeof( level.voteString ),
        "m %d an unforcespec vote against you passed. it was called by ^7%s", clientNum,
      ent->client->pers.netname );
    }
    else
    {
  ...
  else if( !Q_stricmp( arg1, "denybuild" ) )
  {
    ...
    {
      Com_sprintf( level.voteString, sizeof( level.voteString ),
        "m %d a denybuild vote against you passed. it was called by ^7%s", clientNum,
      ent->client->pers.netname );
    }
    else
    {
  ...
  {
    if( !g_extendvotetime.integer || !g_extendvotepercent.integer )
  ...
  else if( !Q_stricmp( arg1, "random_map" ) || !Q_stricmp( arg1, "random_nextmap" ) )
  {
    ...
    else
    {
      ...
      else
      {
        ...
        Com_sprintf( level.voteString, sizeof( level.voteString ),
          "set g_nextMap %s", fileSort[ rand( ) % count ] );
        Com_sprintf( level.voteDisplayString, sizeof( level.voteDisplayString ),
          "Set the ^1next map^7 to a random map" );
      }
  ...
  else
  {
    ...
    trap_SendServerCommand( ent-g_entities, "print \"Valid vote commands are: "
      "map, map_restart, draw, nextmap, kick, mute, unmute, denybuild, allowbuild, poll, sudden_death, extreme_sudden_death/esd, extend, random_map, and random_nextmap\n" );
    return;
  }
  
  if( level.votePassThreshold != 50 )
  {
    if( level.votePassThreshold > 100 )
      level.votePassThreshold = 100;
    else if( level.votePassThreshold < 0 )
      level.votePassThreshold = 0;
  ...
  //G_LogPrintf( "%i %s: %s %s", ent->client->ps.clientNum, arg1 );
...
void Cmd_Destroy_f( gentity_t *ent )
{
  ...
  if( g_noguidnobuild.integer && !Q_stricmp( ent->client->pers.guid, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" ) )
  {
    G_TriggerMenu( ent->client->ps.clientNum, MN_B_NOGUIDNOBUILD );
    return;
  }
...
void Cmd_Buy_f( gentity_t *ent )
{
  ...
  if( weapon != WP_NONE )
  {
    ...
    if( BG_Weapon( weapon )->price > (short)ent->client->pers.Credit && !override )
    {
  ...
  else if( upgrade != UP_NONE )
  {
    ...
    // determine if it costs more than we have
    if( BG_Upgrade( upgrade )->price > (short)ent->client->pers.Credit && !override )
    {
      G_TriggerMenu( ent->client->ps.clientNum, MN_H_NOFUNDS );
      return;
    }
...
void Cmd_Build_f( gentity_t *ent )
{
  ...
  if( level.extremeSuddenDeath && !override )
  {
    trap_SendServerCommand( ent-g_entities,
  ...
  if( level.suddenDeath )
  {
    ...
    if( g_suddenDeathMode.integer == SDMODE_SELECTIVE && !override )
    {
...
void Cmd_PTRCRestore_f( gentity_t *ent )
{
  ...
  if( connection )
  {
    ...
    ent->client->pers.Credit = connection->clientCredit;
    ...
    ent->client->pers.specd = connection->specd;
    ent->client->pers.voteCount = connection->voteCount;
...
void Cmd_PTRCVerify_f( gentity_t *ent )
{
  ...
  if( connection )
  {
    ...
    //  trap_SendServerCommand( ent->client->ps.clientNum, "ptrcrequest" );
...
void Cmd_Share_f( gentity_t *ent )
{
  ...
  if( ent->client->sess.spectatorState == SPECTATOR_FOLLOW )
  {
    trap_SendServerCommand( ent-g_entities, "print \"Stop following to share.\n\"" );
    return;
  }
  ...
  {
    trap_SendServerCommand( ent-g_entities,
      "print \"You cannot share to someone who is following as a spectator\n\"" );
    return;
  }

  targetCredit = level.clients[ clientNum ].pers.Credit;
  playerCredit = ent->client->pers.Credit;
...
void Cmd_Donate_f( gentity_t *ent ) 
{
  ...
  if( ent->client->sess.spectatorState == SPECTATOR_FOLLOW )
  {
    trap_SendServerCommand( ent-g_entities, "print \"Stop following to donate.\n\"" );
    return;
  }
  ...
  while( !donated && value > 0 )
  {
    ...
    for( i = 0; i < level.maxclients; i++ )
    {
      ...
      //check team
      if( thisclient->client->pers.teamSelection != team )
        continue;
      ...
      //make sure they're not following someone as a spec
      if( thisclient->client->sess.spectatorState == SPECTATOR_FOLLOW )
        continue;
      
      //check for max credits
      if( thisclient->client->pers.Credit >= max )
      {
        pwm++;
        continue;
      }
      ...
      if( portion + thisclient->client->pers.Credit >= max )
      {
        thisportion = max - thisclient->client->pers.Credit;
      ...
      if( thisportion > 0 )
      {
        trap_SendServerCommand( i,
          va( "print \"%s^7 donated %s to you, don't forget to say 'thank you'!\n\"",
          ent->client->pers.netname, G_ReadableCredits( thisportion, team ) ) );
      }
    }
  ...
  G_AddCreditToClient( ent->client, (value-total), qfalse );
  
  //tell the donator
  trap_SendServerCommand( ent-g_entities,
    va( "print \"Donated %s to the cause.\n\"",
    G_ReadableCredits( (total-value), team) ) );
}
...
int G_FloodLimited( gentity_t *ent )
{
  ...
  time = level.time + level.pausedTime;

  ent->client->pers.floodDemerits += g_floodMinTime.integer + ent->client->pers.floodTime - time;
...
commands_t cmds[ ] = {
  ...
  { "follow", CMD_DEAD, Cmd_Follow_f },
  { "follownext", CMD_DEAD, Cmd_FollowCycle_f },
  { "followprev", CMD_DEAD, Cmd_FollowCycle_f },
  ...
  { "boost", CMD_TEAM|CMD_LIVING, Cmd_Button6_f }
};
...
void ClientCommand( int clientNum )
{
  ...
  if( cmds[ i ].cmdFlags & CMD_DEAD && ent->client->sess.spectatorState == SPECTATOR_NOT )
  {
    G_TriggerMenu( clientNum, MN_CMD_DEAD );
    return;
  }
...
A short wrapper for those who can't rebind x to +button6.
=================
*/
void Cmd_Button6_f( gentity_t *ent )
{
