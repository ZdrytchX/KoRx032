...
=====================
*/
void SV_DropClient( client_t *drop, int message, char *reason ) {
	int		i;
	challenge_t	*challenge;

        if ( com_developer && com_developer->integer )
          Com_Printf( "SV_DropClient( %i, %i, %s%s%s )\n", (int)(drop - svs.clients), message,
	...
	VM_Call( gvm, GAME_CLIENT_DISCONNECT, drop - svs.clients, message );
        ...
        switch( message )
        {
          ...
          case DROP_INACTIVITY:
            reason = "Dropped due to inactivity";
            break;
          default:
            if( reason == NULL )
              reason = "";
...
static void SV_NextDownload_f( client_t *cl )
{
	...
	SV_DropClient( cl, DROP_DISCONNECT, NULL );
}
...
void SV_UserinfoChanged( client_t *cl ) {
	...
	if( len >= MAX_INFO_STRING )
		SV_DropClient( cl, DROP_OTHER, "userinfo string length exceeded" );
	else
...
static qboolean SV_ClientCommand( client_t *cl, msg_t *msg ) {
	...
	if ( seq > cl->lastClientCommand + 1 ) {
		...
		SV_DropClient( cl, DROP_OTHER, "Lost reliable commands" );
		return qfalse;
	}
...
#ifndef NDEBUG
		SV_DropClient( cl, DROP_OTHER, "DEBUG: illegible client message" );
#endif
		return;
	}
