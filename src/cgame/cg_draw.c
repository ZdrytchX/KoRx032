                                     ...
                                     int textalign, int textvalign )
{
  ...
  switch( primary )
  {
    ...
    default:
      ...
      if( !BG_Weapon( primary )->usesEnergy && (
          BG_InventoryContainsUpgrade( UP_AMMOPACK, ps->stats ) ||
          BG_InventoryContainsUpgrade( UP_BATTLESUIT, ps->stats ) ) )
        maxVal = (int)( 1 + (float)(maxVal) * AMMOPACK_MODIFIER );
      break;
  }
...
static float CG_ChargeProgress( void )
{
  ...
  else if( cg.snap->ps.weapon == WP_ALEVEL3_UPG )
  {
    ...
    max = LEVEL3_POUNCE_TIME_UPG;
           ...
           cg.snap->ps.weapon == WP_ALEVEL4_UPG )
  {
  ...
  else if( cg.snap->ps.weapon == WP_XAEL )
  {
    min = XAEL_CHARGE_TIME_MIN;
    max = XAEL_CHARGE_TIME_MAX;
  }
  if( max - min <= 0.f )
    return 0.f;
                                    ...
                                    qhandle_t shader )
{
  ...
  // Flash red for Xael warning
  if( cg.snap->ps.weapon == WP_XAEL &&
      cg.snap->ps.stats[ STAT_MISC ] >= XAEL_CHARGE_TIME_WARN &&
      ( cg.time & 128 ) )
  {
    color[ 0 ] = 1.f;
    color[ 1 ] = 0.f;
      ...
      if( cg.spectatorPaintX <= rect->x + 2 )
      {
        if( cg.spectatorOffset < cg.spectatorLen )
        {
          ...
          else
          {
            ...
            if( UI_Text_Emoticon( &cg.spectatorList[ cg.spectatorOffset ],
    ...
    vec4_t color, float scale, int textalign, int textvalign, int textStyle )
{
  ...
  else if( !cg.extremesuddendeath )
  {
    if( cg.snap->ps.stats[ STAT_TEAM ] == TEAM_ALIENS )
    {
      float frags = (float)(cgs.alienNextStageThreshold - cgs.alienCredits) / ALIEN_CREDITS_PER_FRAG;
      if( frags < 0.f )
        frags = 0.f;
      ...
      if( cgs.alienNextStageThreshold < 0 )
        ...
        Com_sprintf( s, MAX_TOKEN_CHARS, "Stage %d, %.1f frags for next stage",
...
static void CG_DrawPlayerScore( rectDef_t *rect, float scale, int textalign, vec4_t color )
{
  ...
  float         tx = rect->x, ty = rect->y;
  maxX = rect->x + rect->w;
  text = va( "%d", cg.snap->ps.persistant[ PERS_KILLS ] );
...
static void CG_DrawTeamVote( void )
{
  ...
  s = va( "TEAMVOTE(%i): \"%s\"  [F1]Yes:%i   [F2]No:%i", sec,
          cgs.teamVoteString[ cs_offset ], cgs.teamVoteYes[cs_offset], 
