...
static cvarTable_t cvarTable[ ] =
{
  ...
  { &cg_hudFiles, "cg_hudFiles", "ui/korx-hud.txt", CVAR_ARCHIVE},
  ...
  { &ui_version, "ui_version", "1", CVAR_ROM },
...
static void CG_RegisterClients( void )
{
  ...
  cgs.media.ammopackModel      = trap_R_RegisterModel( "models/players/human_base/ammopack.md3" );
...
void CG_Init( int serverMessageNum, int serverCommandSequence, int clientNum )
{
  ...
  // Make sure we have the right UI VM
  if( strcmp( ui_version.string, GAME_VERSION ) )
    CG_Error( "ERROR: UI and GAME/CGAME VM mismatch. "
              "Some things might not work! "
              "Please remove any extra ui mods you have installed! "
              "Tremfusion comes with one called z-tremfusion-menu-VERSION.pk3!\n" );
}
