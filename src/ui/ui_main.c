...
void UI_LoadHelp( const char *helpFile )
{
  ...
  Com_Printf( "Loading help file '%s'\n", helpFile);
  ...
  while( 1 )
  {
    ...
    if (uiInfo.helpCount == MAX_HELP_INFOPANES) {
      Com_Error (ERR_DROP, "MAX_HELP_INFOPANES\n");
    }
    ...
    if( token.string[0] == '{' )
    {
      ...
      // String_Alloc fails if the file size exeeds STRING_POOL_SIZE (ui_shaded.h)
      // or MEM_POOL_SIZE (ui_shared.c)
...
static void UI_LoadAlienUpgrades( void )
{
  ...
  for( i = PCL_NONE + 1; i < (PCL_NUM_CLASSES-1); i++ )
  {
    ...
    //More UI hax for the spitfire
    if( BG_ClassCanEvolveFromTo( class, PCL_ALIEN_SPITFIRE, credits, stage, 0 , 0 ) >= 0
        && i == PCL_ALIEN_LEVEL1_UPG )
    {
      uiInfo.alienUpgradeList[ j ].text = String_Alloc( BG_ClassConfig( PCL_ALIEN_SPITFIRE )->humanName );
      uiInfo.alienUpgradeList[ j ].cmd =
        String_Alloc( va( "cmd class %s\n", BG_Class( PCL_ALIEN_SPITFIRE )->name ) );
      uiInfo.alienUpgradeList[ j ].type = INFOTYPE_CLASS;
      uiInfo.alienUpgradeList[ j ].v.pclass = PCL_ALIEN_SPITFIRE;
...
void UI_Init( qboolean inGameLoad )
{
  ...
  // report our version
  trap_Cvar_Register( NULL, "ui_version", GAME_VERSION, 0 );
...
void UI_RegisterCvars( void )
{
  ...
  //FIXME: WIRE: I think this is screwing up the TF clients
  // use ui messagemode
