...
void QDECL Com_Printf( const char *fmt, ... ) {
	...
	if (is_new_line && (com_timestamps && com_timestamps->integer)) {
		...
		Com_sprintf(msg, sizeof(msg), "^0[^3%02d:%02d:%02d^0]^7  ", realtime.tm_hour, realtime.tm_min, realtime.tm_sec);
		buf += 20;
	}

	va_start (argptr,fmt);
	Q_vsnprintf (buf, sizeof(msg) - 20, fmt, argptr);
	va_end (argptr);
	is_new_line = msg[strlen(msg) - 1] == '\n';

	// hack to get skipnotify working with timestamps
	if ( buf != msg && !Q_strncmp( buf, "[skipnotify]", 12 ) ) {
		memmove(msg + 12, msg, 20);
...
//	Swap_Init ();
  ...
  // get dedicated here for proper hunk megs initialization
	...
	com_timestamps = Cvar_Get ("com_timestamps", "0", CVAR_ARCHIVE | CVAR_LATCH);
...
void Com_WriteConfigToFile( const char *filename ) {
	...
	FS_Printf (f, "// generated by korx, do not modify\n");
	Key_WriteBindings (f);
