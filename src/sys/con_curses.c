...
#define TITLE "^4---[ ^3KoRx Console ^4]---"
#define PROMPT "^3-> "
...
static void CON_UpdateClock(void)
{
	...
	werase(clockwin);
  if( timestamps )
    CON_ColorPrint(clockwin, va("^0[^3%02d^0%c^3%02d^0%c^3%02d^0]^7 ", realtime.tm_hour, ':', realtime.tm_min, ':', realtime.tm_sec ), qtrue);
  else
    CON_ColorPrint(clockwin, " ", qtrue);
...
void CON_Init(void)
{
  ...
  timestamps = com_timestamps ? com_timestamps->integer : 0;
	...
	inputwin = newwin(1, COLS - Q_PrintStrlen(PROMPT) - (timestamps ? 13 : 1), 
                    LINES - 1, Q_PrintStrlen(PROMPT) + (timestamps ? 13 : 1) );
	input_field.widthInChars = COLS - Q_PrintStrlen(PROMPT) - (timestamps ? 13 : 1);
	if (curses_on) {
	...
	// Create the clock
	clockwin = newwin(1, (timestamps ? 15 : 1), LINES - 1, 0);
	CON_UpdateClock();
